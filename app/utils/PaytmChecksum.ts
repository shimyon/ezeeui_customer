...
export class PaytmChecksum{
    ...
    static encrypt(input, key) {
		...
		var cipher = crypto.AES.encrypt(input, key, {iv:iv});
		// var encrypted = cipher.update(input, 'binary', 'base64');
		// encrypted += cipher.final('base64');
		var encrypted=cipher.ciphertext.toString(crypto.enc.Base64);
		return encrypted;
	}
	...
	static decrypt(encrypted, key) {
		...
		// return decrypted;
		var iv = crypto.enc.Hex.parse("@@@@&&&&####$$$$")
		var decipher = crypto.AES.decrypt(encrypted, key, {iv:iv});
		var decrypted=decipher.words;
		return decrypted;
	}
	...
	static generateRandomString(length) {
		return new Promise(function (resolve, reject) {
			 var salt=crypto.lib.WordArray.random((length * 3.0) / 4.0);
			 resolve(salt);
			 reject("error generateString")
		});
	}
	...
	static calculateHash(params, salt) {		
		...
		return crypto.SHA256(finalString).toString(crypto.enc.Hex) + salt;
	}
